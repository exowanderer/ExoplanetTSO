...
PACKAGENAME = metadata.get('package_name', 'exotso')
DESCRIPTION = metadata.get(
    'description', 'ExoplanetTSO: Package for optimal exoplanet & brown dwarf time series observation data analysis'
)
AUTHOR = metadata.get('author', 'Jonathan Fraine')
AUTHOR_EMAIL = metadata.get('author_email', 'jdfraine@gmail.com')
LICENSE = metadata.get('license', 'MIT')
URL = metadata.get('url', 'https://github.com/exowanderer/ExoplantTSO')
...
if sys.version_info < tuple((int(val) for val in __minimum_python_version__.split('.'))):
    sys.stderr.write(
        f"ERROR: ExoplantTSO requires Python {__minimum_python_version__} "
        "or later\n"
    )
    sys.exit(1)
...
VERSION = metadata.get('version', '0.3')
...
for root, dirs, files in os.walk(PACKAGENAME):
    c_files.extend(
        ...
        for filename in files if filename.endswith('.c')
    )
